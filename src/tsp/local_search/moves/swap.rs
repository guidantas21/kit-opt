...
pub fn best_improvement(solution: &mut Solution, data: &ProblemData) -> bool {
    ...
    for i in 1..route.path.len() - 2 {
        ...
        let next_node_i = route.path[i + 1];

        let delta_i = -data.cost(prev_node_i, node_i) - data.cost(node_i, next_node_i);
        ...
        for j in i + 1..route.path.len() - 1 {
            ...
            if i + 1 == j {
                delta += data.cost(node_j, node_i);
            } else {
                delta += data.cost(node_j, next_node_i) + data.cost(prev_node_j, node_i)
                    - data.cost(prev_node_j, node_j);
            }
            delta += data.cost(prev_node_i, node_j) + data.cost(node_i, next_node_j)
...
fn test_objective(solution: &Solution, delta: i32, i: usize, j: usize) {
    ...
    solution_after_move.routes[0].path.swap(i, j);

    let expected_objective = solution_after_move.calculate_cost();

    assert_eq!(
        move_objective, expected_objective,
        "Invalid swap evaluation at ({i}, {j})! Expected: {expected_objective}, but got {move_objective}",
    );
}
